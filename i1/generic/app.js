...
const store = urlParams.get('store') || 'marios_pizza';

const STORAGE_KEY = `${store}_data`;
const DATA_URL = `stores/${store}/data.json`;
const STYLES_URL = `stores/${store}/styles.css`;

let menuData = null;

// Load store-specific stylesheet
function loadStoreStyles() {
  document.getElementById('store-styles').href = STYLES_URL;
}
  ...
  try {
    ...
    if (!menuData) {
      document.getElementById('menu-container').innerHTML =
        `<p class="error">Unable to load menu for "${store}". Please check the store parameter.</p>`;
    }
